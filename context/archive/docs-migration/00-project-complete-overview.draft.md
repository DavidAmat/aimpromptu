# AImpromptu (aitu) — Complete Project Overview  (DRAFT — finalized in Phase 1 as context/00-project-complete-overview.md)

> One-shot orientation. Paste this alone to get the whole project. Verify against code before promoting
> out of the kit. Renames assumed applied (Phase 0): `aitu-backend`, `aitu-frontend`, module `aitu_backend`.

## What it is

AImpromptu turns a **custom text music notation** into rendered **sheet music**. Two independent
services under one repo:
...
text uses `__` to split right/left per line. (Two-hand support is in progress in the working tree.)
...
- Frontend: functional React + hooks, ESLint flat config (typescript-eslint + react-hooks + react-refresh),
  music logic isolated under `src/music/`, rendering isolated in `PianoSheet.tsx`. READMEs preserve
  "nuances we got wrong before" — keep that knowledge.
