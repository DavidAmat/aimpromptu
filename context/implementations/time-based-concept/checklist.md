...
- [x] P7.13 `[AITU]` **Added, not in `plan.md`.** Pick a stretch of columns with Shift, pick a set of notes with Command, and two draggable toolboxes — *the Frames one gives a passage its own key signature; the Notes one is the empty panel its controls will live in*
